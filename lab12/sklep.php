...
<?php
    ...
    {
        $add='
        <form method="post" name="AddToCardForm" enctype="multipart/form-data" action="' . htmlspecialchars($_SERVER['REQUEST_URI']) . '">
        <input type="number" name="quantity" value="1" min="1" max="100" placeholder="Quantity" required>
        <input type="hidden" name="product_id" value=$id_prod>
    ...
    function removeFromCard($produkt)
    {

        $stmt = $this->link->prepare("DELETE FROM `koszyk` WHERE `id` = ? LIMIT 1");
        $stmt->bind_param("i", $produkt_id);
        $stmt->execute();
        $stmt->close();
    }
